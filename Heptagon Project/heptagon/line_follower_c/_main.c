...
int main(int argc, char** argv) {
  ...
  while ((!(step_max)||(step_c<step_max))) {
    ...
    if ((scanf("%d", &irsenLeft)==EOF)) {
      return 0;
    };;
    
    if ((scanf("%d", &irsenFront)==EOF)) {
      return 0;
    };;
    
    if ((scanf("%d", &irsenRight)==EOF)) {
      return 0;
    };;
    Line_follower__main_step(sen0, sen1, sen2, sen3, sen4, irsenLeft,
                             irsenFront, irsenRight, &_res, &mem);
    printf("%d\n", _res.v_l);
    printf("%d\n", _res.v_r);
    printf("%d\n", _res.direction);
    fflush(stdout);
